...
try:
    old_sites = []
    with open(f"output-{model_name}-stance-persona.jsonl", "r") as f:
        for line in f:
...
def get_completion(site, topic, model=model_name):
    system_prompt = f"You are an assistant to help the user determine the stance of a website on a public figure personality or a topic."
    prompt = f"Is {{0}} pro-{{1}}. \nAssistant must return the stance and the reason in JSON format, and do not provide anything else.".format(site,topic)
    ...
    try:
        ...
        while True and i < 5:
            try:
                ...
                ### check if response object doesnt have the required keys stance and reason,
                if 'stance' not in response.keys() or 'reason' not in response.keys():
                    raise Exception("Response object doesnt have the required keys stance and reason")
                break                
...
topics = ['Trump', 'Biden', 'Putin', 'American', 'Russian', 'German', 'Palestinian']

with open(f"output-{model_name}-stance-persona.jsonl", "a") as f:
